...
* **Workflow:**
    1.  **Data Loading:** Reads 2 csv files.
    2.  **Text Preprocessing:** Cleans and prepares text data for vectorization.
    3.  **Text Classification:**
        * Trains and evaluates Linear SVC and Random Forest models using a TF-IDF.
    4.  **Similarity Search (LSH):**
        * Implements **MinHash** to create signatures for text documents.
        * Builds a **MinHashLSH** index for efficient querying.
    5.  **Baseline Comparison:**
        * Implements a brute-force similarity search using nearest neighbors (with TF-IDF vectors) to serve as a ground truth.
    6.  **Evaluation:**
...
* **Workflow:**
    1.  **Data Parser:** Includes a custom parser function to convert string representations of lists into clean arrays of floats.
    2.  **DTW Implementation:**
        * The core logic is in the `dtw_distance` function.
    3.  **Optimizations:**
        * **Numba:** The decorator is used to Just-in-Time (JIT) compile the DTW function, dramatically speeding up the operations.
        * **Sakoe-Chiba Band:** Implements a window constraint to reduce the computational complexity, making it feasible for long series.
    4.  **Execution:**
        * The main script reads a CSV file containing pairs of time series.
        * It iterates through each pair, applying the optimized DTW distance algorithm to calculate their similarity.
